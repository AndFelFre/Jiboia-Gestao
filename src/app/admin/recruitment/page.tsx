...
export default function RecruitmentPage() {
    ...
    async function loadData() {
        try {
            ...
            console.error('[RecruitmentPage] Error loading data:', error)
        } finally {
            setLoading(false)
        }
    }
    ...
    return (
        <div className="min-h-screen bg-[#FDFDFD]">
            <header className="bg-white border-b border-slate-100 flex items-center justify-center sticky top-0 z-50">
                <div className="max-w-7xl w-full px-8 py-6 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        <Link href="/admin" className="w-10 h-10 bg-slate-50 text-slate-400 rounded-xl flex items-center justify-center hover:bg-slate-900 hover:text-white transition-all">
                            <ArrowLeft size={20} />
                        </Link>
                        ...
                        <div>
                            <h1 className="text-lg font-black text-slate-900 tracking-tight">Recrutamento <span className="text-primary">& Seleção</span></h1>
                            <p className="text-[9px] uppercase font-black tracking-[0.2em] text-slate-400">Hub de Talentos</p>
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <Button asChild variant="ghost" className="rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-slate-900 hover:text-white gap-3 h-12 px-6 font-black uppercase tracking-widest text-[10px] transition-all">
                            <Link href="/admin/recruitment/kanban">
                                <LayoutGrid className="w-4 h-4" /> Kanban
                            </Link>
                        </Button>
                        <Button asChild className="rounded-2xl h-12 px-6 bg-primary hover:bg-slate-900 text-white transition-all font-black uppercase tracking-widest text-[10px] gap-3 shadow-lg shadow-primary/20">
            ...
            <main className="max-w-7xl mx-auto px-8 py-12 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                {/* KPI Analytics Block */}
                ...
                <div className="space-y-8">
                    <div className="flex items-center justify-between px-4">
                        ...
                        <Button asChild variant="link" className="text-primary font-black uppercase text-[10px] tracking-widest gap-2">
                            <Link href="/admin/recruitment/jobs">Ver todas <ChevronRight className="w-3 h-3" /></Link>
                        </Button>
                    </div>
                    ...
                    {jobs.length === 0 ? (
                        <div className="bg-white p-20 rounded-[3rem] border-4 border-dashed border-slate-100 flex flex-col items-center text-center">
                            ...
                            <h3 className="text-xl font-black text-slate-900 mb-2">Sem vagas ativas</h3>
                            <p className="text-slate-400 font-medium max-w-sm mb-8">Inicie um novo processo seletivo para atrair talentos.</p>
